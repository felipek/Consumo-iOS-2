...
- (NSInteger)tableView:(UITableView *)table numberOfRowsInSection:(NSInteger)section
{
	if (section == 0)
		...
		return [[[Services singleton] allServices] count];
	else
		return [thanks count];
}
...
- (UITableViewCell *)tableView:(UITableView *)table cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	switch (indexPath.section) {
		case 0:
			switch (indexPath.row) {
				...
				case 1:
					...
					[cell.detailTextLabel setText:@"2011"];
					break;
		...
		case 1:
		{
			NSString *service = [[[Services singleton] allServices] objectAtIndex:indexPath.row];
			NSDictionary *definition = [[Services singleton] serviceWithName:service];
			[cell.textLabel setText:[definition objectForKey:@"Name"]];
			[cell.detailTextLabel setText:[definition objectForKey:@"Version"]];
			break;
		}
