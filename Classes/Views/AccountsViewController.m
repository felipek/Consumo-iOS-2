...
- (void)serviceDefinitionUpdateThread
{
	NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
	[[Services singleton] definitionsUpdate];
	[pool release];
}

- (void)viewWillAppear:(BOOL)animated
{
	if (definitionSetup == NO) {
		[[Services singleton] definitionsLoad];
		definitionSetup = YES;
		
		NSThread *threadUpdate = [[NSThread alloc] initWithTarget:self selector:@selector(serviceDefinitionUpdateThread) object:nil];
		[threadUpdate start];
		[threadUpdate release];
	}
